...
python training_SA3.py --experiment {experiment} --task {task} --num_training {num_training} --N_EPOCHS {N_EPOCHS} --seed {seed} --layer_len {layer_len} {calibrate_flag} --num_calibrations {num_calibrations} --iter_calibrations {iter_calibrations} {c2st_flag}
...
def create_training_cMAD_job_script(experiment, task, num_training, N_EPOCHS, seed, layer_len):
    job_script = f"""#!/bin/bash
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=04:00:00
...
"""
    ...
    job_file_path = os.path.join(output_dir, f"{task}_cMAD_{num_training}_{seed}.sh")
    with open(job_file_path, 'w') as f:
        f.write(job_script)
    print(f"Job script created: {job_file_path}")
